...
const char* EXTERNAL_FILE = "treeFile.txt";
int n;
/* Генерация случайного значения типа unsigned long long.
Максимальный размер unsigned long long - FFFF FFFF FFFF FFFF или 18 446 744 073 709 551 615. В treeUtils.h задана
константа максимального значения(4 294 967 295), для того, чтобы несколько ограничить диапазон для генерации. */
unsigned long long llrand() {
    unsigned long long r = 0;

    /* Запись long long занимает 64 бита, а стандартный оператор rand() позволяет генерировать значение лишь до 32767 (15 бит).
    Для покрытия всех битовых значений, используется побитовый сдвиг(15 раз за итерацию).
    По завершению цикла, биты полученного значения обрезаются, в соответствии с максимальным значением. */
...
tnode* makeRandomTree() {
    ...
    cout << "Количество генераций случайного числа: ";
    cin >> n;
    ...
    cout << "Генерирация случайного дерева...\n";
    return tree;
}
...
void exportTreeToFile(tnode* tree) {
    ...
    cout << "Экспорт дерева в файл " << EXTERNAL_FILE << "...\n\n";
}
...
tnode* importTreeFromFile() {
    ...
    fin.close();
    cout << "Импорт дерева из файла " << EXTERNAL_FILE << "...\n";
    return tree;
}
...
unsigned long long getSumOfAllChilds(tnode* tree) {
    if (tree != NULL) {
        ...
        if (tree->left != NULL) {
            ...
            leftSum = (tree->left->sum + tree->left->value) / 2;
        }

        if (tree->right != NULL)
        {
            tree->right->sum = getSumOfAllChilds(tree->right);
            rightSum = (tree->right->sum + tree->right->value) / 2;
        }
    ...
    if (arg->tree != NULL) {
        ...
        if (arg->tree->left != NULL) {
            ...
            if (leftCreateStatus != 0) {
                cout << "[ОШИБКА] Не удается создать поток. Статус: " << leftCreateStatus << endl;
        ...
        if (arg->tree->right != NULL) {
            ...
            if (rightCreateStatus != 0) {
                cout << "[ОШИБКА] Не удается создать поток. Статус: " << rightCreateStatus << endl;
                exit(ERROR_CREATE_THREAD);
            }
        }
        ...
        if (leftCreateStatus != SUCCESS) {
            cout << "[ОШИБКА] Не удается добавить поток. Статус: " << leftCreateStatus << endl;
            exit(ERROR_JOIN_THREAD);
        }
        if (arg->tree->left != NULL) {
            arg->tree->left->sum = leftArg.tree->sum;
            leftSum = (arg->tree->left->sum + arg->tree->left->value) / 2;
        }

        rightCreateStatus = pthread_join(rightThread, (void**)&rightJoinStatus);
        if (rightCreateStatus != SUCCESS) {
            cout << "[ОШИБКА] Не удается добавить поток. Статус: " << rightCreateStatus << endl;
            exit(ERROR_JOIN_THREAD);
        }
        if (arg->tree->right != NULL) {
            arg->tree->right->sum = rightArg.tree->sum;
            rightSum = (arg->tree->right->sum + arg->tree->right->value) / 2;
        }

        arg->tree->sum = (leftSum + rightSum) / 2;
    }
    return 0;
}
